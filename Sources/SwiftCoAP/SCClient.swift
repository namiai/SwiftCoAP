...
public class SCClient {
    ...
    // CONSTANTS
    let kMaxObserveOptionValue: UInt = 8_388_608
    ...
    public func sendCoAPMessage(_ message: SCMessage, endpoint: NWEndpoint) {
        let currentMessageId = transportLayerObject.getMessageId(for: endpoint)
    ...
    fileprivate func sendCoAPMessageOverTransportLayerWithData(_ message: SCMessage, endpoint: NWEndpoint, notifyDelegateAfterSuccess: Bool = false) {
        do {
            try transportLayerObject.sendCoAPMessage(message, toEndpoint: endpoint, token: currentToken, delegate: self)
        ...
        } catch let SCCoAPTransportLayerError.sendError(errorDescription) {
            notifyDelegateWithTransportLayerErrorDescription(errorDescription)
        } catch let SCCoAPTransportLayerError.setupError(errorDescription) {
            notifyDelegateWithTransportLayerErrorDescription(errorDescription)
        } catch {}
    }
...
extension SCClient: SCCoAPTransportLayerDelegate {
    public func transportLayerObject(_: SCCoAPTransportLayerProtocol, didReceiveData data: Data, fromEndpoint endpoint: NWEndpoint) {
    ...
    public func transportLayerObject(_: SCCoAPTransportLayerProtocol, didFailWithError error: NSError) {
